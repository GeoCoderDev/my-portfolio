...
const Header = () => {
  ...
  return (
    <header
      id="Header"
      className={`w-[100%] h-[15dvh] bg-[#ffffff80] text-black border-solid border-b-[1px] border-black/[.20] flex backdrop-blur-[10px] items-center justify-center px-[2.5vw] select-none fixed top-0 left-0 z-[100]`}
    >
      <a
        className={`xs:h-[40%] sm:h-[60%] flex items-end justify-center /border-2 absolute left-[2.5vw]`}
        href="/"
      >
        <img className="h-[100%] bg-cover" src={Logo} alt="Logo Geocoder" />
      </a>

      <NavBar className="xs:hidden sm:hidden md:flex  lg:flex w-[40%] /border-2 justify-self-center flex items-center justify-between lg:text-[1.2rem] sm:text-[1rem]" />

      <div
        className={`lg:w-36 sm:w-32 /border-2 absolute right-[2.5vw] flex items-center justify-between`}
      >
      ...
      <div
        className={`flex flex-col items-center justify-start transition-all duration-200 bg-black text-white absolute top-0 left-0 w-[100%] h-[100dvh]  ${
          openedMenu ? " opacity-100 z-[101]" : " opacity-0 z-[-1]"
        }`}
      >
        <NavBar
